...
#            pprint.pprint(fileattrs)
    ...
    def get_remote_files_dir(self):
        ...
        try:
            #print "starting getting file"
            fileattrs = self.transport.get_remote_files_dir(self.remote_dir)
#            pprint.pprint(fileattrs)
        except Exception, e:
            self.lg.error("Failed to get list of files in remote directory %s : %s" % (self.remote_dir, str(e)))
            self.lg.debug(traceback.format_exc())
        ...
        for filepath in filelist:
            ...
            try:
                if not self.donotdownload:                
                    ok = self.transport.process_file(filepath,self.regexes,self.notfoundre)
                else:
                    ok = True
            except Exception, e:
